...
### FINDING-PENTEST-001 [HIGH] — TLS 1.0 accepted at Cloudflare edge → **CLOSED 2026-04-22**

Originally curl-verified: `curl --tlsv1.0 --tls-max 1.0 https://accessbridge.space/` returned HTTP 200 (CF edge allowed TLS 1.0 handshakes; modern baseline is TLS 1.2 minimum).

- **Tooling:** [tools/ops/enforce-min-tls.sh](../../tools/ops/enforce-min-tls.sh) + [docs/operations/cloudflare-hardening.md](../operations/cloudflare-hardening.md). Idempotent bash script: resolves the Cloudflare zone id, reads current `min_tls_version`, PATCHes to `1.2` if different, verifies via `curl --tlsv1.0 --tls-max 1.0` probe. Supports `--token-file <path>` (mode ≤ 0o600, symlink-refused, content validated) as the preferred token source.
- **Blast-radius:** `accessbridge.space` is its own Cloudflare zone; `min_tls_version` is a zone-level setting affecting ONLY this domain. The shared-Caddy comment in the initial audit applied to the Caddy origin (`ti-platform-caddy-1`); Cloudflare is the CDN layer in front and is per-domain. No coordination with other shared-origin project owners was needed.
- **Execution (2026-04-22):** Token provisioned via scoped Custom Token (`Zone.Zone Settings: Edit` on `accessbridge.space`, 1-day TTL). Script run: `bash tools/ops/enforce-min-tls.sh --token-file ~/.cf-min-tls-token` → CF API confirmed `min_tls_version: 1.0 → 1.2` via PATCH. Propagation took ~60 s. Verification: `curl --tlsv1.0 --tls-max 1.0 https://accessbridge.space/` now exits with schannel `SEC_E_UNSUPPORTED_FUNCTION` (curl exit 35) — handshake cryptographically refused. CF API re-read confirms `min_tls_version = 1.2`. TLS 1.2 continues to return HTTP 200. Token file scrubbed via `shred -u` immediately after.
- **Probe-logic fix in script:** the initial run surfaced a cosmetic bug in `probe_tls10()` — when curl refused the handshake, `%{http_code}` wrote `000` and the `|| echo "0"` fallback appended another `0`, producing `0000`. The caller's `[[ "$code" == "0" ]]` check missed that, reporting "still open" after a successful rejection. Fixed by separating curl exit-status capture from stdout and treating empty / all-zeros / non-zero-exit as `REJECTED`. Re-run of `--verify-only` after the fix: exit 0 + "TLS 1.0 handshake REJECTED".
- **Nightly regression probe:** [.github/workflows/cve-watch.yml](../../.github/workflows/cve-watch.yml) `tls-min-version` job runs `enforce-min-tls.sh --verify-only` daily and opens a security issue if TLS 1.0 is ever accepted again (rogue Page Rule override, dashboard config drift, CF default rollback).
- **Status:** **CLOSED**.
