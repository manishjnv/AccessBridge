...
- **`./deploy.sh`** — NOT RUN. Destructive external effects (push to origin/main, rsync to VPS, nginx route exposure for `/api/pilot/*`) gated on user approval. The commit below tagged `feat(deploy):` will trigger auto-bump 0.22.0 → 0.23.0 on deploy; auto-bump + re-zip + push + rsync + container restart + health check will run then.
- **VPS-side `PILOT_ADMIN_TOKEN` env var** — must be provisioned on the VPS before `POST /api/pilot/enrollments` accepts admin-gated calls. Currently returns 503 until the observatory container's environment includes this variable. Documented in [docs/deployment/team.md](docs/deployment/team.md) — admin-side setup step.
...
No new RCA entries — consistent with Session 23 policy: adversarial findings S25-1 / S25-2 / S25-3 are pre-production defense-in-depth caught before deploy, not regressions of shipped code. The existing RCA BUG-013 (prune-on-write), BUG-015 (proto-pollution), BUG-017 (chmod-after-write race), BUG-018 (symlink refusal), BUG-019 (umask regression), BUG-010 (Cloudflare cache-bust) patterns are all **re-honored** in the new code — confirmed by targeted greps in Phase 3.
